...
def _topk(heatmap, K=40, num_classes=1):
    ...
    # topk_clses = custom_replace(topk_clses, 0, 0) ###################################################### temp
...
# Newly modified mot_decode for centre extraction
def mot_decode_centre(heatmap,
               wh,
               reg=None,
               num_classes=2,
               cat_spec_wh=False,
    ...
    # Centres added for coordinate transform
    centres = torch.cat([xs, ys, classes], dim=2)
    detections = torch.cat([bboxes, scores, classes], dim=2)

    return detections, inds, cls_inds_masks, centres
