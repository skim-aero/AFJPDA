...
# state includes x, y postion, its velocity, w, h, and its velocity.
...
def initialisation(path1, path2, param, model):
    ...
    if len(detections_one) and len(detections_two):
        ...
        XYZ[0][2] = [i for i in detections_one[3]]  # wi of " ([2] is zi)
        XYZ[0][3] = [i for i in detections_one[4]]  # hi of "

        # second frame
        XYZ[1][0] = [i for i in detections_two[0]]  # xi of second frame
        XYZ[1][1] = [i for i in detections_two[1]]  # yi of "
        XYZ[1][2] = [i for i in detections_two[3]]  # wi of " ([2] is zi)
        XYZ[1][3] = [i for i in detections_two[4]]  # hi of "
...
def JPDA_ini(detections_one, model, param): #, numlines):
    ...
    Class = [i for i in detections_one[5]]
    ...
    return Xe, Pe, N_cnt, Term_Con, Tent_Con, We, He, Classe, Tracke, Track_idx