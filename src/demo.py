...
device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu') # space deleted between : and 0
...
from track import eval_seq, eval_seq_JPDA, eval_seq_AFJPDA, eval_imgs_output_dets
...
def run_demo(opt):
    ...
    # set device
    opt.device = str(0) #str(FindFreeGPU()) Forcing to use gpu 0
    ...
    try:
        if opt.input_mode == 'video':
            if opt.id_weight == 1:
                print( "\
                       **************************************\n\
                       Running Multi-Class FairMOT (Original)\n\
                       **************************************\n")
                eval_seq(opt=opt,
                         ...
                         mode='track')
            elif opt.id_weight == 2:
                print( "\
                       **************************\n\
                       Running Multi-Class JPDA\n\
                       **************************\n")
                eval_seq_JPDA(opt=opt,
                              data_loader=data_loader,
                              data_type='mot',
                              result_f_name=result_file_name,
                              save_dir=frame_dir,
                              ...
                              frame_rate=frame_rate,
                              mode='track')
            elif opt.id_weight == 3:
                print( "\
                       **************************\n\
                       Running Multi-Class AFJPDA\n\
                       **************************\n")
                eval_seq_AFJPDA(opt=opt,
                              data_loader=data_loader,
                              data_type='mot',
                              result_f_name=result_file_name,
                              save_dir=frame_dir,
                ...
                print( "\
                       ***********************************\n\
                       Running Dectection only (CentreNET)\n\
                       ***********************************\n")
                eval_seq(opt=opt,
                         data_loader=data_loader,
                         data_type='mot',
