...
from lib.tracker.multitracker import JDETracker, MCJDETracker, JPDATracker, AFJPDATracker, id2cls
    ...
    for cls_id in range(num_classes):  # process each object class
        ...
        for frame_id, tlwhs, track_ids, scores in cls_results:
            for tlwh, track_id, score in zip(tlwhs, track_ids, scores):
                ...
                x1, y1, w, h = tlwh
                temp = [frame_id, track_id, x1, y1, w, h]
                fin_results.append(temp)
    
    fin_results.sort(key = lambda i: i[1])
    fin_results.sort(key = lambda i: i[0])

    with open(file_name, 'w') as f:
        for i in range(len(fin_results)):
            line = save_format.format(frame=fin_results[i][0], id=fin_results[i][1],
                                        x1=fin_results[i][2], y1=fin_results[i][3], 
                                        w=fin_results[i][4], h=fin_results[i][5])
            f.write(line)

    logger.info('save results to {}'.format(file_name))


# Newly defined for JPDA tracker
def write_results_JPDA(output_dir, results):
    f = open(output_dir, 'w')

    results = results.tolist()
    results.sort(key = lambda i: i[6])
    results.sort(key = lambda i: i[0])

    for row in results:
        # # [FrameID, x_img, y_img, w, h, , x_local, y_local. z_local, classID, TrackID]
        # print('%d,%.2f,%.2f,%.2f,%.2f,%.3f,%.3f,%.3f,%d,%d' % (
        #     row[0], row[1], row[2], row[3], row[4], row[7], row[8], row[9], row[5], row[6]), file=f)

        # # [FrameID, TrackID, x_1, y_1, w, h, 1 (score), classID]
        # print('%d,%d,%.6f,%.6f,%.6f,%.6f,%d,%d' % (
        #     row[0], row[6], row[1]-row[3]/2, row[2]-row[4]/2, row[3], row[4], 1, row[5]), file=f)
        
        # [FrameID, TrackID, x_1, y_1, w, h, -1 (score), -1 (x), -1 (y), -1 (z)]
        print('%d,%d,%.6f,%.6f,%.6f,%.6f,%d,%d,%d,%d' % (
            row[0], row[6], row[1]-row[3]/2, row[2]-row[4]/2, row[3], row[4], -1, -1, -1, -1), file=f)


# Newly defined function for JPDA
def logging_postprocess(final_result, Conf_idx):

    temp = np.empty((0, 9))

    for index in Conf_idx:
        temp = np.append(temp, np.array([final_result[index]]), axis=0)
...
def eval_seq(opt,
        ...
        if mode == 'track':  # process tracking
            ...
            for cls_id in range(opt.num_classes):  # process each class id
                ...
                for track in online_targets:
                    tlwh = track.tlwh
                    t_id = track.track_id + cls_id*1000
                    score = track.score
    ...
    logger.info('Prossesed frames {} (avg {:.2f} fps)'.format(frame_id, frame_id / timer.total_time))
    write_results_dict(result_f_name, results_dict, data_type)

    return frame_id, timer.average_time, timer.calls


# Newly defined for JPDA tracker (Calculate velocity from first two frames)
def eval_seq_JPDA(opt,
             data_loader,
             data_type,
             result_f_name,
             save_dir=None,
    ...
    tracker = JPDATracker(opt, frame_rate)
    ...
    for path, img, img0 in data_loader:
        ...
        if frame_id == 0:
            # update detection results of this frame(or image)
            frame_1 = tracker.update_detection(blob, img0)
            frame_id += 1
        elif frame_id == 1:    # JPDA initialising using first two frames
            # update detection results of this frame(or image)
            frame_2 = tracker.update_detection(blob, img0)
            frame_id += 1
        else:
            temp = 3
            datasets.LoadVideo.release_video(data_loader, temp) # Release video to start from first frame again
            break
    ...
    for path, img, img0 in data_loader:
        ...
        if frame_id == 0:
            ...
            Xe, Pe, cls_feats, fin_result, N_cnt, Term_con, Tent_con, Conf_idx, Term_idx, Tent_idx, Track_idx, frame_tmp = \
                tracker.update_tracking(blob, img0, frame_1, frame_2, 
                                        [], [], [], [], [], [], [], [],
                                        [], [], [], [], [], [], [])
            timer.toc()
            # -----

            Old_Conf_idx = copy.deepcopy(Conf_idx)
            New_Conf_idx = copy.deepcopy(Conf_idx)
            
        else:
            # ----- track updates of each frame
            timer.tic()

            len_det = len(fin_result)
            We = np.reshape(fin_result[:,2], (len_det, 1))
            He = np.reshape(fin_result[:,3], (len_det, 1))
            ClassID = np.reshape(fin_result[:,4], (len_det, 1))
            TrackID = np.reshape(fin_result[:,5], (len_det, 1))

            Xe, Pe, cls_feats, fin_result, N_cnt, Term_con, Tent_con, Conf_idx, Term_idx, Tent_idx, Track_idx, frame_tmp = \
                tracker.update_tracking(blob, img0, frame_1, frame_2, 
                                        Xe, Pe, We, He, ClassID, TrackID, cls_feats, N_cnt, 
                                        Term_con, Tent_con, Conf_idx, Term_idx, Tent_idx, Track_idx, frame_tmp)

            timer.toc()
            # -----

            for no in Conf_idx:
                if no not in Old_Conf_idx:
                    New_Conf_idx = np.append(New_Conf_idx, no)

        # Draw track/detection
        if show_image or save_dir is not None:
            if frame_id >= 0:
                online_im: ndarray = vis.plot_tracks_JPDA(image=img0,
                ...
                final_result = logging_postprocess(fin_result, Conf_idx)
        ...
        # Append result dictionary for track logic
        results = []
        results = fin_result
        results = np.insert(results, 0, frame_id+1, axis=1)

        if frame_id == 0:
            results_lgic = results
        else:
            results_lgic = np.concatenate((results_lgic, results), axis=0)

        # Append result dictionary for write result
        results_temp = []
        results_temp = final_result
        results_temp = np.insert(results_temp, 0, frame_id+1, axis=1)

        if frame_id == 0:
            results_dict = results_temp
        else:
            results_dict = np.concatenate((results_dict, results_temp), axis=0)

        # Append one tentative measurement to log if the track confirmed
        for i in range(len(results_lgic)):
            if results_lgic[i][6] - 1 in New_Conf_idx and results_lgic[i][0] == frame_id:
                results_dict = np.concatenate((results_dict, [results_lgic[i]]), axis=0)

        # Update old confirmed track index and empty the new confirmed track index
        for no in New_Conf_idx:
            if no not in Old_Conf_idx:
                Old_Conf_idx = np.append(Old_Conf_idx, New_Conf_idx)
    ...
    # Write track/detection results
    logger.info('Prossesed frames {} (avg {:.2f} fps)'.format(frame_id, frame_id / timer.total_time))
    results_dict = np.unique(results_dict,axis=0)
    write_results_JPDA('../results/results.txt', results_dict)

    return frame_id, timer.average_time, timer.calls


# Newly defined for AFJPDA tracker (Calculate velocity from first two frames)
def eval_seq_AFJPDA(opt,
             data_loader,
             data_type,
             result_f_name,
             save_dir=None,
    ...
    tracker = AFJPDATracker(opt, frame_rate)
